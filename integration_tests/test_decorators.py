...
def test_once_decorator():
    ...
    def _sync_function(arg1: int, arg2: int):
        sleep(2)
        return arg1 + arg2
...
async def test_once_decorator_async_function():
    ...
    async def _async_function(arg1: int, arg2: int, initial: bool = True) -> Any:
        res = None
        if initial:
...
@pytest.mark.asyncio
async def test_async_once_decorator_raise_on_locked(init_cachify_fixture: Any):
    @once(key='async_test_key-{arg1}-{arg2}', raise_on_locked=True)
    async def _async_function(arg1: int, arg2: int) -> int:
...
def test_cached_decorator_sync_function():
    @cached(key='test_key')
    def _sync_function_wrapped(arg1: int, arg2: int):
        return arg1 + arg2
...
async def test_cached_decorator_async_function():
    ...
    async def _async_function_wrapped(arg1: int, arg2: int):
        return arg1 + arg2
